...
export async function createExcel(data, empresaData) {
  if (data.length > 0) {
    new Promise((resolve, reject) => {
      ...
      if (empresaData.UUID === 935 || empresaData.UUID === 950) {

        for (const [index, value] of HEADERS_LAUREATE.entries()) {
          ws.cell(1, index + 1)
            .string(value)
            .style(HEADERS_STYLE);
        }
        ...
        for (let a = 0; a < data.length; a++) {
          ...
          ws.cell(a + 2, 12)
            .number(data[a]["Acuse ISR (Retenciones por Salarios)"])
            .style(
              STYLE_STATUS(data[a]["Acuse ISR (Retenciones por Salarios)"])
            );

          ws.cell(a + 2, 13)
            .number(data[a]["Pago ISR (Retenciones por Salarios)"])
            .style(
              STYLE_STATUS(data[a]["Pago ISR (Retenciones por Salarios)"])
            );
          ...
          ws.cell(a + 2, 17)
            .number(data[a]["Opinion cumplimiento IMSS / INFONAVIT"])
            .style(
              STYLE_STATUS(data[a]["Opinion cumplimiento IMSS / INFONAVIT"])
            );
        }
        ...
        for (let a = 0; a < data.length; a++) {
          ...
          ws.cell(a + 2, 11)
            .number(data[a]["ODC (Orden de Compra)"])
            .style(STYLE_STATUS(data[a]["ODC (Orden de Compra)"]));
